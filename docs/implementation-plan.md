...
イベントはイベントとして配信する。各イベントには、イベント処理後のスナップショットを含める。
...
```text
Running / Paused: 100 ms
NotRunning / Ended: 500 ms
```

タイマー表示用途まで担う場合は、後から周期を短くする。Auto Splitter同期のみであれば、イベント通知が即時に行われるため、定期配信を高頻度にする必要はない。
