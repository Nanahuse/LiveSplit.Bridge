...
from .client import BridgeClient, BridgeClientError, GAME_TIME_OPERATIONS, TIMER_OPERATIONS
...
def parser() -> argparse.ArgumentParser:
    ...
    result.add_argument("--rpc-endpoint", default=os.getenv("LIVESPLIT_BRIDGE_RPC_ENDPOINT", DEFAULT_RPC_ENDPOINT))
    result.add_argument("--event-endpoint", default=os.getenv("LIVESPLIT_BRIDGE_EVENT_ENDPOINT", DEFAULT_EVENT_ENDPOINT))
    result.add_argument("--timeout", type=float, default=3.0, help="RPC timeout in seconds (default: 3)")
    result.add_argument("--json", action="store_true", help="Print protobuf messages as JSON")
    ...
    game_time.add_argument("operation", choices=["initialize", "set", "pause", "resume"])
    game_time.add_argument("seconds", type=float, nargs="?", help="Game time in seconds (required by set)")

    events = commands.add_parser("events", help="Continuously monitor bridge events")
    events.add_argument("--count", type=int, help="Exit after receiving this many events")
    return result
...
def snapshot_lines(snapshot: common_pb2.TimerSnapshot) -> list[str]:
    ...
    real_time = format_ticks(snapshot.real_time_ticks) if snapshot.HasField("real_time_ticks") else "-"
    game_time = format_ticks(snapshot.game_time_ticks) if snapshot.HasField("game_time_ticks") else "-"
    return [
        f"session={snapshot.session_id} sequence={snapshot.event_sequence} revision={snapshot.state_revision}",
        f"phase={phase} split={snapshot.split_index}/{snapshot.split_count} paused={snapshot.is_paused}",
        f"real_time={real_time} game_time={game_time} game_time_initialized={snapshot.is_game_time_initialized}",
    ]
...
def run_events(endpoint: str, as_json: bool, count: int | None) -> int:
    ...
    try:
        while count is None or received < count:
            event = bridge_pb2.BridgeEvent.FromString(socket.recv())
            if as_json:
    ...
    try:
        with BridgeClient(args.rpc_endpoint, round(args.timeout * 1000)) as client:
            if args.command == "attach":
                ...
                print_message(response if args.json else response.attach.snapshot, args.json)
            elif args.command == "snapshot":
                response = client.snapshot()
                print_message(response if args.json else response.get_snapshot.snapshot, args.json)
                ...
                if args.operation == "set":
                    ...
                    if args.seconds is not None:
                        parser().error(f"game-time {args.operation} does not accept seconds")
