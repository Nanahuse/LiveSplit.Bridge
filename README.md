...
イベントストリームは1秒ごとにスナップショットを含まないハートビートを配信します。
ハートビートの `event_sequence` は最後に送信を試みた通常イベントの番号を示し、
ハートビート自身では増加しません。定期フルスナップショットは30秒ごとに配信します。
クライアントは3秒間ハートビートを受信できない場合、`session_id` が変わった場合、または
`event_sequence` の欠落を検出した場合に、RPCで最新スナップショットを取得してください。
